...
function Navbar() {
  const NavbarItem = [
    { name: "How it started?", link: "#service" },
    { name: "Tokenomic", link: "#gallery" },
    { name: "Buy Loafcat", link: "#about" },
    { name: "Partnership", link: "#about" },
  ]

  const [toggleMenu, setToggleMenu] = React.useState(false)

  return (
    <nav className="w-full absolute mx-auto top-0 left-0 z-50 bg-transparent">
      <div className="max-w-7xl mx-auto px-8 py-4 flex justify-between items-center ">
        ...
        <ul className="hidden md:flex space-x-8 md:space-x-12">
          {NavbarItem?.map((menu, index) => (
              <li
                key={index}
                className=" text-white text-md lg:text-lg hover:text-gray-300 transition duration-300"
              >
                <a className='fontMain' href={menu?.link}>{menu?.name}</a>
              </li>
            ))}
        </ul>
        ...
        {toggleMenu && (
            <div className="w-full md:hidden bg-main flex justify-center h-screen right-0 top-0 px-4 absolute z-20">
              <div className="">
                <AiOutlineClose
                fontSize={28}
                className="text-white cursor-pointer absolute top-6 right-4"
            ...
            <ul className="flex items-center flex-col pt-20 space-y-6">
              ...
              <li
                key={index}
                className=" text-white px-4 text-md"
              >
